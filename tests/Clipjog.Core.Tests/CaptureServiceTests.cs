...
using Clipjog.Core.Tests.Fakes;
using Xunit;
...
public sealed class CaptureServiceTests : IDisposable
{
    ...
    private ClipjogSettings _settings = new();

    public CaptureServiceTests()
    {
        _root = Path.Combine(Path.GetTempPath(), "clipjog-capture-tests", Guid.NewGuid().ToString("n"));
    ...
    {
        _settings = new ClipjogSettings { MonitorClipboard = false };
    ...
    {
        _settings = new ClipjogSettings { IgnoredProcesses = ["keepass.exe"] };
    ...
    {
        _settings = new ClipjogSettings { StoreImages = false };
    ...
    {
        _settings = new ClipjogSettings { MaxClips = 3 };
