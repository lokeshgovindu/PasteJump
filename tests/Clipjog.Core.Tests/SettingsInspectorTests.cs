...
namespace Clipjog.Core.Tests;

/// <summary>
/// The Advanced settings inventory. Reflection-based, so it needs tests: a change to
/// <see cref="ClipjogSettings"/> cannot break it at compile time, only at runtime.
/// </summary>
public sealed class SettingsInspectorTests
{
    ...
    {
        var rows = SettingsInspector.Describe(new ClipjogSettings());

        Assert.Contains(rows, r => r.Name == nameof(ClipjogSettings.MaxClips));
        Assert.Contains(rows, r => r.Name == nameof(ClipjogSettings.Theme));
        Assert.Contains(rows, r => r.Name == nameof(ClipjogSettings.GridDensity));
        Assert.Contains(rows, r => r.Name == nameof(ClipjogSettings.IgnoredProcesses));
    }
    ...
    public void Computed_views_over_other_settings_are_excluded()
    {
        ...
        Assert.DoesNotContain(rows, r => r.Name == nameof(ClipjogSettings.PasteModeOptions));
    }
    ...
    {
        var settings = new ClipjogSettings { MaxClips = 999 };

        var row = SettingsInspector.Describe(settings).Single(r => r.Name == nameof(ClipjogSettings.MaxClips));
    ...
    public void An_enum_lists_its_legal_values_as_its_type()
    {
        ...
        var row = SettingsInspector.Describe(new ClipjogSettings())
            .Single(r => r.Name == nameof(ClipjogSettings.Theme));
    ...
    public void Empty_values_are_labelled_rather_than_blank()
    {
        ...
        Assert.Equal("(empty)", rows.Single(r => r.Name == nameof(ClipjogSettings.IgnoredProcesses)).Value);
    }
    ...
    public void The_default_formatter_id_is_stored_explicitly_not_as_null()
    {
        ...
        var row = SettingsInspector.Describe(new ClipjogSettings())
            .Single(r => r.Name == nameof(ClipjogSettings.DefaultFormatterId));
    ...
    {
        var settings = new ClipjogSettings { DefaultFormatterId = "  " };
        settings.Normalise();
    ...
    public void An_unknown_formatter_id_is_preserved_rather_than_reset()
    {
        ...
        var settings = new ClipjogSettings { DefaultFormatterId = "some-future-formatter" };
        settings.Normalise();
    ...
    {
        var settings = new ClipjogSettings { IgnoredProcesses = ["keepass.exe", "1password.exe"] };

        var row = SettingsInspector.Describe(settings)
            .Single(r => r.Name == nameof(ClipjogSettings.IgnoredProcesses));
