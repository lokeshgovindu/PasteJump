...
public sealed class ConsecutiveDuplicateTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "clipjog-dedup-tests", Guid.NewGuid().ToString("n"));
    ...
    {
        _settings = new ClipjogSettings { AllowDuplicateClips = true };
    ...
    public void A_suppressed_duplicate_still_reports_that_a_copy_happened()
    {
        ...
        // Ctrl+C is indistinguishable from Clipjog having missed the copy entirely - and the user's
