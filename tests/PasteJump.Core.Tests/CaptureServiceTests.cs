...
public sealed class CaptureServiceTests : IDisposable
{
    ...
    /// <summary>
    /// Reported 2026-08-21 as pressing Ctrl+V in Edge showing the paste overlay and a copy notification
    /// immediately after it. The paste's own clipboard write was not recognised as ours, so it was stored as a
    /// brand new clip and announced - a duplicate clip and a history row per paste, and the browse position
    /// reset each time.
    /// <para>
    /// The cause is that a clipboard write does not read back as what was written. PasteJump deliberately drops
    /// the formats Windows derives from <c>CF_UNICODETEXT</c>, and Windows then regenerates them from the
    /// <em>pasting</em> thread's locale rather than the copying application's - so a clip captured under English
    /// (India) reads back with English (US) in its <c>CF_LOCALE</c>. Same characters, same lengths, four
    /// different bytes, and a different hash over everything.
    /// </para>
    /// <para>
    /// The bytes below are the ones measured in the live store: <c>0x4009</c> as captured against <c>0x0409</c>
    /// as synthesised. This test fails against a build that identifies its own writes by content hash.
    /// </para>
    /// </summary>
    [Fact]
    public void APasteIsRecognisedAsOursEvenWhenWindowsRefillsTheLocale()
    {
        // What the clip holds, and therefore what the paste hands to the clipboard.
        var written = LocalisedTextSnapshot("clip pasted in Edge", [0x09, 0x40, 0x00, 0x00]);

        // What the clipboard reports afterwards: the same text, with CF_LOCALE regenerated by Windows.
        var readBack = LocalisedTextSnapshot("clip pasted in Edge", [0x09, 0x04, 0x00, 0x00]);

        _clipboard.EnqueueRead(readBack);
        _selfWrites.NoteWrite(written.SelfWriteKey);
        ...
        // The two halves of what the user saw: no clip added, and nothing announced.
        Assert.Equal(0, captured);
        Assert.Equal(0, notices);
    }

    /// <summary>
    /// A text clip carrying the derived formats a real one does - the ANSI and OEM renderings and the locale.
    /// </summary>
    private static ClipboardSnapshot LocalisedTextSnapshot(string text, byte[] locale)
    {
        ClipPayload[] payloads =
        [
            new(13, null, System.Text.Encoding.Unicode.GetBytes(text + ' ')),
            new(1, null, System.Text.Encoding.ASCII.GetBytes(text + ' ')),
            new(7, null, System.Text.Encoding.ASCII.GetBytes(text + ' ')),
            new(16, null, locale),
        ];
