...
using Clipjog.App.Views;
using Clipjog.Core;
using Clipjog.Core.Capture;
using Clipjog.Core.Formatting;
using Clipjog.Core.Imaging;
using Clipjog.Core.Model;
using Clipjog.Core.PasteMode;
using Clipjog.Core.Settings;
using Clipjog.Core.Storage;
using Clipjog.Interop;

namespace Clipjog.App;
...
{
    private const string SingleInstanceMutexName = @"Global\Clipjog.SingleInstance.9F2C41A6";

    private Mutex? _singleInstanceMutex;

    private AppPaths _paths = null!;
    private SettingsStore _settingsStore = null!;
    private ClipjogSettings _settings = null!;
    ...
    private ClipjogPasteHost _pasteHost = null!;
    ...
    private void Compose()
    {
        ...
        _pasteHost = new ClipjogPasteHost(
            _store,
            _clipboard,
            new InputSender(),
            _selfWrites,
    ...
    /// onto the dispatcher by <see cref="ClipjogPasteHost"/>.
    /// </summary>
    ...
    /// theme, not Clipjog's own: Windows defaults to light apps on a dark taskbar, and using the app
    ...
    private void OnSettingsApplied(ClipjogSettings updated)
    {
        _settings = updated;
    ...
    {
        var text = $"Clipjog {AppVersion.Display}";
    ...
    private void OnClipEditorRequested(Clip clip)
    {
        try
        {
            ...
            if (text is null)
            {
                MessageBox.Show("This clip has no text to edit.", "Clipjog");
                return;
            }

            var tempPath = Path.Combine(Path.GetTempPath(), $"clipjog-{clip.Id}.txt");
    ...
    private void OnExportRequested(Clip clip)
    {
        ...
        try
        {
            ...
            if (clip.Kind == ClipKind.Image)
            {
                ...
                if (bitmap is null)
                {
                    MessageBox.Show("This image could not be exported.", "Clipjog");
                    return;
                }
