...
namespace Clipjog.Core.Storage;
...
public sealed class ClipStore : IDisposable
{
    ...
    public ClipStore(AppPaths paths, IClock? clock = null)
    {
        ...
        Execute(ClipjogSchema.Pragmas);
        Execute(ClipjogSchema.Ddl);
        SetMeta("schema_version", ClipjogSchema.CurrentVersion.ToString(CultureInfo.InvariantCulture));
    }
